...
We not have enough information to look at the prior for the heights themselves. This can be thought of as the plausible range of heights before the model has seen any data.
...
If we recall that $h_i ~ Normal(\mu_i = \alpha + \beta x_i, \sigma)$, then we reframe our uncertainty as just possible values of mean values of height ($\mu_i$) for a given weight ($x_i$). For example, the distribution of possible mean values of height when weight is 50 kilograms, is computed by,
