...
The book has some good information about spurious correlations on pages 134-135 and is worth a read. The basic argument is that if the "real" predictor is included in the model, then multivariate regression will do a good job figuring out which one is "real", even if other variables are included that are also driven by the "real" predictor. Whereas a model can never tell you *what variable you should have included in the model* and we will be left thinking that the covariates with large absolute values of the coefficients are actually driving our response variable, when it could just be spurious correlation.
...
The regression model is able to "discover" that the mean of `y` is actually `xpos-xneg`, and after it controls for one covariate it is able to show that the effect is actually greater for each covariate when combined than when seperated.
...
The model is saying that the effect of the treatment is small, but the effect of the original height and the amount of fungus are important. It is answering the question, *once we know that a plant has fungus, does soil treatment matter?*, and the answer to that is no, because soil treatment effected growth by reducing fungus. If we want to know the effect of the treatment on the growth then we need to omit fungus, the post-treatment variable.
...
The categorical varaible basically creates a different linear model for each Clade. So for Ape, the model is equal to $\mu_i = \alpha$, for `r unique(d$clade)[2]` the model is $\mu_i = \alpha + \beta*nwm_i$...etc. 
...
Because $\alpha$ is the average milk energy for apes, we can calculate the posterior distribution  of average milk energy in each category using samples:

```{r}
# sample the posterior
post <- extract.samples(m5.13)

# compute averages for each county
milk.energy <- post %>%
  mutate(ape=a,
         nwm=a+b.nwm,
         owm=a+b.owm,
         s=a+b.s) %>%
  select(ape,nwm,owm,s)
```

```{r, echo=F, fig.width=3.5,fig.heigh=3}
ggplot(melt(milk.energy)) + 
  geom_boxplot(aes(variable,value),width=0.2,color="black",fill="grey70",outlier.size = 0.5) +
  theme_bw() +
  labs(x="category",y="average milk energy")
```

**Unique intercepts using an index variable**

Previously we added a new column for each clade. We can accomplish the same thing by adding a single column with different index values. These will come up more later for multi-level models.
...
precis(m5.14, depth=2)
```

Compare the $\alpha$ values above to the values in the boxplot. Earilier we had to add each estimate to the $\alpha$ estimate to calculate the mean value for each category. Using an index variable skips this additional step. 

**Ordinary least squares**

The book has a section (page 159-162) on how to use `lm` in R to create linear regression models. I do not go into that here, however, there is a nice `rethinkng` function to create `MAP` models from model design formulas.
...
Add LDS data to divorce rate model:

```{r}
data(WaffleDivorce)
d <- WaffleDivorce

# add percent LDS from wikipedia
...
d2 <- d %>%
  select(divorce = Divorce,marriage=Marriage,
         age=MedianAgeMarriage,pct_LDS) %>%
  mutate(marriage = scale(marriage),
         age=scale(age),
         pct_LDS=scale(pct_LDS))

hw5m4 <- map(
  alist(
    divorce ~ dnorm(mu,sigma),
    mu <- a + bR*marriage + bA*age + bM*pct_LDS,
    a ~ dnorm(0,100),
...
Now fit a bivariate model and plot both while holding the other variable at its mean.



These homework examples use the `foxes` dataset:
...
Fit two linear regressions (1) $bodyweight=f(area)$ and (2) $bodyweight=f(groupsize)$, and plot regression line with 95% interval of the mean.
...
area.seq <- seq(min(d$area)-0.5, max(d$area)+0.5,0.1)

# calculate 95%% HPDI around mean
mu <- link(m_area, data=data.frame(area=area.seq,n=1000))
mu.mean <- apply(mu,2,mean)
mu.HPDI <- apply(mu,2,HPDI,prob=0.95)

# plot
ggplot() + geom_point(aes(d$area, d$weight), shape=21) + theme_bw() +
  geom_line(aes(area.seq,mu.mean), color="dodgerblue") +
  geom_ribbon(aes(x=area.seq,ymin=mu.HPDI[1,], ymax=mu.HPDI[2,]), alpha=0.5) +
  labs(x="area",y="weight")

# GROUP SIZE
# define groupsize sequence
gsize.seq <- seq(min(d$groupsize)-1, max(d$groupsize)+1,1)

# calculate 95% HPDI around mean
mu <- link(m_gsize, data=data.frame(groupsize=gsize.seq,n=1000))
mu.mean <- apply(mu,2,mean)
mu.HPDI <- apply(mu,2,HPDI,prob=0.95)

# plot
ggplot() + geom_point(aes(d$groupsize, d$weight), shape=21) + theme_bw() +
  geom_line(aes(gsize.seq,mu.mean), color="dodgerblue") +
  geom_ribbon(aes(x=gsize.seq,ymin=mu.HPDI[1,], ymax=mu.HPDI[2,]), alpha=0.5) +
  labs(x="group size",y="weight")
```

**(5H2)**

Fit a bivariate model and plot each predictor vs the response while holding the other predictor at its mean.
...
plot groupsize while area is held at it's mean:
...
pred.data <- data.frame(area=area.avg, groupsize=gsize.seq)
...
# simulate counterfactual divorce outcomes
m.sim <- sim(m5h2, data=pred.data, n=1e4)
m.PI <- apply(m.sim, 2, PI,prob=0.95)
```

```{r, echo=F}
ggplot() + geom_line(aes(gsize.seq,mu.mean), color="dodgerblue") +
  geom_ribbon(aes(x=gsize.seq,ymin=mu.PI[1,], ymax=mu.PI[2,]), alpha=0.5) +
  geom_ribbon(aes(x=gsize.seq,ymin=m.PI[1,], ymax=m.PI[2,]), alpha=0.2) +
  labs(x="group size when [area = mean(area)]",y="predicted weight") +
  theme_bw()
```

plot area while groupsize is held at it's mean:
...
pred.data <- data.frame(area=area.seq, groupsize=gs.avg)
...
```{r, echo=F}
ggplot() + geom_line(aes(area.seq,mu.mean), color="dodgerblue") +
  geom_ribbon(aes(x=area.seq,ymin=mu.PI[1,], ymax=mu.PI[2,]), alpha=0.5) +
  geom_ribbon(aes(x=area.seq,ymin=m.PI[1,], ymax=m.PI[2,]), alpha=0.2) +
  labs(x="area when [group size = mean(group size)]",y="predicted weight") +
  theme_bw()
```

**(5H3)**

The question is intented to show what happens when correlated predictors are included in the model. I went through the chapter example in detail and refer my future self to that section to understand the effects of correlated predictors.
